...
export const MemeProvider = ({ children }) => {
    ...
    const getTemplates = async () => {
        const response = await fetch('http://127.0.0.1:5000/api/get-meme-templates')
    ...
    const getTemplate = async (id) => {
        const response = await fetch(`http://127.0.0.1:5000/api/get-meme-template/${id}`)
        const data = await response.json()
        setTemplate(data)
        console.log(data)
        setLoading(false)
    ...
    const saveMeme = async (meme, inputs, positioning) => {
        ...
        const response = await fetch('http://127.0.0.1:5000/api/add-meme', {
            method: 'POST',
            headers: {
    ...
    const getMyMemes = async (token) => {
        console.log(token)
        const response = await fetch('http://127.0.0.1:5000/api/view-my-memes', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
    ...
    const getMyMeme = async (id, token) => {
        console.log(token)
        const response = await fetch(`http://127.0.0.1:5000/api/view-meme/${id}`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
    ...
    const updateMyMeme = async (meme, inputs, positioning) => {
        ...
        const response = await fetch(`http://127.0.0.1:5000/api/update-meme/${meme.id}`, {
            method: 'PATCH',
            headers: {
                'Content-Type': 'application/json'
            },
    ...
    const deleteMeme = async (id) => {
        const token = auth.currentUser.uid
        await fetch(`http://127.0.0.1:5000/api/delete-meme/${id}`, {
            method:'POST',
            headers: {
                'Content-Type': 'application/json'
            },
