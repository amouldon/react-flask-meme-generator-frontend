...
if (dragTarget){
  ...
  const calculatePosition = (textbox, div) => {
    ...
    const mathStuffHeight = 1 - (divHeight / imgHeight)
    const mathStuffWidth = 1 - (divWidth / imgWidth)
    console.log(divWidth)
    console.log(imgWidth)
    console.log(mathStuffWidth)
    const distanceFromTop = (imgY) + ((imgHeight * originalPercentageFromTop) * mathStuffHeight)
    const distanceFromLeft = (imgX) + ((imgWidth * originalPercentageFromLeft) * mathStuffWidth)
    ...
    <div className="row mt-5 d-flex justify-content-center">
      ...
      <div className="col-5 mx-3 py-3 d-flex flex-column justify-content-around" on>
        <div className="dark-border-no-hover p-3 mb-3 d-flex flex-column">
          <p className="h5" id='text-instructions'>Enter your meme's labels into the text field below, and then press the button to generate a textbox that can be dragged by double clicking and holding, you are alotted a maximum of four text boxes. Click a textbox to edit, press in a textbox to remove it.</p>
        </div>
